...
public:
    ...
    World(const std::string &config_file) :
        road(config_file)
        {
            ...
            YAML::Node obstacles_config = config["static"]["obstacles"];
    ...
    road::Road get_road() const
    {
        return road;
    }

    std::vector<entity::Agent> get_agents() const
    {
        return agents;
    }

    entity::Agent get_agent(const int &agent_id) const
    {
